...
public class BudgetDAO extends GenericDAO<Budget> {
    ...
    public Budget getOneById(int id) throws SQLException {
        String sql = "SELECT id, expense_cat, max_amount, goal_amount, date_start, date_end FROM budgets WHERE id = ?";
    ...
    public boolean update(int id, Budget obj) throws SQLException {
        String sql = "UPDATE budgets SET expense_cat = ?, max_amount = ?, goal_amount = ?, date_start = ?, date_end = ? WHERE id = ?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            throw new SQLException("Error Updating Budget: " + e.getMessage(), e);
        }
    }
    ...
    public void insert(Budget obj) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            throw new SQLException("Error inserting in budgets: " + e.getMessage(), e);
        }
    }
